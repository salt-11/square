...
@RestController
@RequestMapping("/api/yl/lvs/pay")
public class SumPayYlController {
	...
	public Object recharge(RechargeDto rechargeDto) {
		...
		TDeptRateChannel deptRateChannel = deptRateChannelService.findByDeptIdAndChannel(rechargeDto.getPartnerId(),"sum");
		...
		TMchInfoChannel mchInfoChannel = mchInfoChannelService.findByMchIdAndChannel(rechargeDto.getMchId(), "sum");
		...
		String longitude = "";
		String latitude = "";
		if(deptRateChannel.getChannelNo().equals("101243663")) {
			List<TMerPool> merPoolList = merPoolServic.findByPC(rechargeDto.getProvinceCode(), rechargeDto.getCityCode(),1);
			if(merPoolList.size() == 0) {
				orderStatus = 3;
				returnMsg = "订单上送的地区码交易不支持";
				//throw new RestException(401, "订单上送的地区码交易不支持1!");
			}else {
		...
		// = NumberUtil.round(NumberUtil.mul(rechargeDto.getOrderAmount(), mchRate),0,RoundingMode.UP).longValue();
		...
		//平台
		//Long costFee = NumberUtil.parseLong(NumberUtil.roundStr(orderAmount*(Double.valueOf(deptRateChannel.getChannelCostRate())), 0,RoundingMode.UP));
		Long costFee = NumberUtil.round(NumberUtil.mul(rechargeDto.getOrderAmount(), deptRateChannel.getChannelCostRate()),0).longValue();
		...
		if(payOrder.getOrderStatus() == 1) {
			Map<String,String> resMap = sumChannel.recharge(rechargeDto, orderId.toString(), mchInfoChannel.getOutMchId(), mchCardChannel.getProtocol(),channelMerNo,shareAmount.toString(),deptRateChannel.getChannelNo(),deptRateChannel.getChannelMerAppId(),userIpAddr,longitude,latitude);
	...
	public Object query(QueryDto queryDto) {
		...
		TDeptRateChannel deptRateChannel = deptRateChannelService.findByDeptIdAndChannel(queryDto.getPartnerId(),"sum");
		if(deptRateChannel == null) {
			throw new RestException(401, "未找到渠道信息");
		}
		//签名信息校验
