...
import cn.hawy.quick.modular.api.dto.ff.OrderNotifyDto;
...
import cn.hawy.quick.modular.api.validate.ff.FfPayValidate;
...
@RestController
@RequestMapping("/api/ff/pay")
public class FfPayController {
	...
	public Object recharge(RechargeDto rechargeDto) {
		...
		//参数校验
		FfPayValidate.recharge(rechargeDto);

		if(NumberUtil.compare(Double.parseDouble(rechargeDto.getOrderAmount()),Double.parseDouble(ffProperties.getMinOrderAmount())) < 0) {
		...
		if(dept.getChannelType() == 1){
			deptRateChannel = deptRateChannelService.findByDeptIdAndChannel(rechargeDto.getPartnerId(), "ff");
		}else {
			deptRateChannel = deptRateChannelService.findByDeptIdAndBankCodeAndChannel(rechargeDto.getPartnerId(), mchCard.getBankCode(), "ff");
		}
		...
		if(NumberUtil.compare(Double.parseDouble(rechargeDto.getMchRate()),Double.parseDouble(deptRateChannel.getCostRate())) < 0) {
		...
		//计算方式和后端通道保持一致
		//商户
		Long mchFee = NumberUtil.round(NumberUtil.mul(rechargeDto.getOrderAmount(), rechargeDto.getMchRate()),0).longValue();
		Long mchAmount = orderAmount - mchFee;
		//渠道
		Long deptFee = NumberUtil.round(NumberUtil.mul(rechargeDto.getOrderAmount(), deptRateChannel.getCostRate()),0).longValue();
		...
		payOrder.setMchRate(rechargeDto.getMchRate());
		...
		Map<String,String> resMap = ffChannel.preOrderCreate(String.valueOf(orderId), rechargeDto.getOrderAmount(), rechargeDto.getCardNo(), mchCard.getMobile(), rechargeDto.getMchRate(), mchInfo.getCustomerName(), mchInfo.getCustomerIdentNo(), rechargeDto.getCity(), mchCard.getCvn(), mchCard.getExpired(), rechargeDto.getMcc());
	...
	public Object query(QueryDto queryDto) {
		...
		if(payOrder.getOrderStatus() == 1) {
			Map<String,String> resMap = ffChannel.queryOrderStatus(String.valueOf(payOrder.getOrderId()));
			if("1".equals(resMap.get("orderStatus"))) {
				//不做处理
			}else if("2".equals(resMap.get("orderStatus"))) {
				boolean orderStatusFlag = payOrderService.updateOrderStatusSuccess(payOrder, dept,resMap.get("returnMsg"));
	...
	public Object orderNotify(OrderNotifyDto orderNotifyDto) {
		log.info("FF交易通知请求-orderNotify={}",JSON.toJSONString(orderNotifyDto));
		String md5Str = "data="+orderNotifyDto.getData()
						+"&merchantId="+orderNotifyDto.getMerchantId()
						+"&key="+ffProperties.getKey();
		String checkSign = SecureUtil.md5(md5Str);
		if(!checkSign.equals(orderNotifyDto.getSign())) {
			log.info("FF交易通知返回-orderNotify={}","验签失败返回fail");
			return "fail";
		}
		String dataDecrypt = ffChannel.rsaDecrypt(orderNotifyDto.getData());
		HashMap<String, String> dataMap = FfChannel.form2Map(dataDecrypt);
		//获取订单信息
		TPayOrder payOrder = payOrderService.getById(dataMap.get("merOrderNumber"));
		if(payOrder == null) {
			//什么都不做
			log.info("FF交易通知返回-orderNotify={}","订单不存在返回fail");
			return "fail";
		}
		//渠道商信息
        Dept dept = deptService.getById(payOrder.getDeptId());
        if(dept == null) {
            log.info("FF交易通知返回-orderNotify={}","渠道信息不存在返回fail");
            return "fail";
        }
		...
		if(payOrder.getOrderStatus() == 1) {
			...
			if(flag) {
				...
				log.info("FF交易通知返回-orderNotify={}","状态修改失败返回fail");
                return "fail";
			}
		}
		String signContent = MapUtil.joinIgnoreNull(MapUtil.sort(map), "&", "=");
	    ...
	    log.info("FF交易通知返回-orderNotify={}","success");
	    return "success";
	}
